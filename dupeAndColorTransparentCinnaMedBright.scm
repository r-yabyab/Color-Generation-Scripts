(define (add-layers-transparent-cinnamedbright)
...
;(- (random 151) 50) ;-50 to 100
        ...
        (gimp-colorize (car (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE)))))) (random 360) 100 43)
        (gimp-colorize (car (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE))))))) (random 360) (random 100) 53)
        (gimp-colorize (car (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE)))))))) (random 360) 100 49)
        (gimp-colorize (car (cdr (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE))))))))) (random 360) (random 100) 43)
        (gimp-colorize (car (cdr (cdr (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE)))))))))) (random 360) (random 100) -40)
...
(script-fu-register
  "add-layers-transparent-cinnamedbright"
  "<Image>/Colors/DupelicateAndColorTransparentCinnaMedbright"
