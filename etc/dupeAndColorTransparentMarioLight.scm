(define (add-layers-transparent-mario-light)
...
(let loop ((x 0) (y 0) (iteration 0))
  (if (< iteration 30)
      (begin
        ...
        (gimp-colorize (car (cdr (cdr (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE)))))))))) (random 360) (random 100) (random 100))
        (gimp-colorize (car (cdr (cdr (cdr (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE))))))))))) (random 360) (random 100) (random 100))
        (gimp-colorize (car (cdr (cdr (cdr (cdr (cdr (cdr (cdr (cdr (vector->list (cadr (gimp-image-get-layers SF-DRAWABLE)))))))))))) (random 360) (random 100) (random 100))
        ...
        (loop (if (= (remainder iteration 10) 9) 0 (+ x 250))
              (if (= (remainder iteration 10) 9) (+ y 500) y)
              (+ iteration 1))
...
(script-fu-register
  "add-layers-transparent-mario-light"
  "<Image>/Colors/DupelicateAndColorTransparentMarioLight"
